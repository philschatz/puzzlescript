...
class Engine extends eventemitter2_1.EventEmitter2 {
    ...
    setLevel(levelNum) {
        ...
        return lodash_1._.flattenDeep(this.currentLevel);
    }
    tick() {
        let changes = [];
        ...
        this.currentLevel.forEach(row => {
            row.forEach(cell => {
                this.gameData.rules.forEach(rule => {
                    ...
                    const mutators = lodash_1._.flattenDeep(rule.getMatchedMutatorsOrNull(cell) || []);
        ...
        return lodash_1._.flattenDeep(changes).filter(change => !!change); // Some rules only have actions and return null. Remove those from the set
    }
    pressUp() { }
    pressDown() { }
    pressLeft() { }
