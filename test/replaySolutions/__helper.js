...
function createTestForGame (gistName) {
    if (process.env['SKIP_SOLUTIONS']) {
        describe.skip(`Skipping replay tests for ${gistName}`, () => {
            it.skip('skipping test')
        })
        console.log('Skipping Replay tests')
        return
    }
    describe(`replays levels of "${gistName}"`, () => {
        const solutionFilename = path.join(__dirname, '../../gist-solutions', `${gistName}.json`)

        it(`plays the solved levels of ${gistName} ${gistName === CYBER_LASSO ? '(just the last level)' : ''}`, async () => {
            const gistFilename = path.join(__dirname, `../../gists/${gistName}/script.txt`)
            const { engine, data } = parseEngine(fs.readFileSync(gistFilename, 'utf-8'))
            let recordings = JSON.parse(fs.readFileSync(solutionFilename, 'utf-8')).solutions

            // In the interest of speed, just test the last level of CyberLasso because it is so long
            if (gistName === CYBER_LASSO) {
            ...
            saveCoverageFile(data, gistFilename, `${gistName}-playgame`)
        })
    })
}

module.exports = { createTestForGame }
